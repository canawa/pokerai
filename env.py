...
class PokerEnv:
    ...
    def reset(self): # сбрасываем стек и все остальное
        self.hands = cards.draw_pocket_cards()
        ...
        self.score = []
        self.done = False
        self.player_hand_one_hot = cards.cards_to_one_hot(self.player_hand) # one hot вектор для своей руки
        self.villain_hand_one_hot = cards.cards_to_one_hot(self.villain_hand) # one hot вектор для руки противника
    ...
    def step(self,action): # действие (пуш или фолд) пуш = 1 фолд = 0
        if action == 0: # фолд
            self.reward = -0.5
            self.done = True
            return self.reward, self.done, self.player_hand_one_hot, self.villain_hand_one_hot
        elif action == 1: # пуш (играем тут префлоп ток)
            self.flop = cards.draw_flop() # заранее раздаем, но не даем эту инфу модели
            self.turn = cards.draw_turn() # заранее раздаем, но не даем эту инфу модели
            self.river = cards.draw_river() # заранее раздаем, но не даем эту инфу модели

            self.flop_one_hot = cards.cards_to_one_hot(self.flop) # one hot вектор для флопа
            self.turn_one_hot = cards.cards_to_one_hot(self.turn) # one hot вектор для терна
            self.river_one_hot = cards.cards_to_one_hot(self.river) # one hot вектор для ривера

            self.score = cards.get_score()
